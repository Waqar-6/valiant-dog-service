package com.wfarooq.dog_service.dog.controller;
...
public class DogController {
    ...
    public ResponseEntity<ResponseDto> createDog (@Valid @RequestBody CreateDogRequest request) {
        ...
        return new ResponseEntity<>(new ResponseDto(StatusConstants.STATUS_201, DogConstants.MESSAGE_201), HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<ResponseDto> updateDog (@Valid @RequestBody CreateDogRequest request, @PathVariable UUID dogId) {
        ...
        return isUpdated ? new ResponseEntity<>(new ResponseDto(StatusConstants.STATUS_200, DogConstants.MESSAGE_200_UPDATE), HttpStatus.OK) :
                new ResponseEntity<>(new ResponseDto(StatusConstants.STATUS_417, DogConstants.MESSAGE_417_UPDATE), HttpStatus.EXPECTATION_FAILED);
    }

    @DeleteMapping("/{dogId}")
    public ResponseEntity<ResponseDto> updateDog (@PathVariable UUID dogId) {
        boolean isDeleted = dogService.deleteDog(dogId);
        return isDeleted ? new ResponseEntity<>(new ResponseDto(StatusConstants.STATUS_200, DogConstants.MESSAGE_200_DELETE), HttpStatus.OK) :
                new ResponseEntity<>(new ResponseDto(StatusConstants.STATUS_417, DogConstants.MESSAGE_417_DELETE), HttpStatus.EXPECTATION_FAILED);
    }
